...
export class ExternalMapService extends BaseShardService {
  ...
  async fetchAndStoreMapData() {
    try {
      ...
      await Promise.all([
        ...data.intersections.map((intersection: Omit<Intersection, "shardKey">) => this.deleteIntersection(intersection)),
        ...data.roads.map((road: Omit<ExternalRoad, "shardKey">) => this.deleteRoad(road))
      ]);
  ...
  async deleteIntersection(data: Omit<Intersection, 'shardKey'>) {
    ...
    try {
      await connection.execute(
        'DELETE FROM Intersection WHERE name = ?',
        [data.name]
      );
    } finally {
      await connection.end();
  ...
  async deleteRoad(data: Omit<ExternalRoad, 'shardKey'>) {
    ...
    try {
      await connection.execute(
        'DELETE FROM ExternalRoad WHERE start_intersection = ? AND end_intersection = ?',
        [data.start_intersection, data.end_intersection]
      );
    } finally {
      await connection.end();
    }
  ...
  async findAllIntersections() {
   ...
   // fetch from external API
    ...
    const data = await response.json();
    const results = data.intersections;
